/**
 * gw-userOrgSelector.js : 人员（组织机构）选择工具。
 * version : v0.1.0
...
(function ($) {
    // 在调用页面添加赋值回调
    window.addUserOrgToList = function (id) {
        $('#btnClose' + id + 'Modal').click();
        $('body').loadingUI({ text: '数据加载中...' });
    }
   
    $.fn.GWUserOrgSelector = function (options) {
        var defaultVal = {
        ...
        return this.each(function () {
            ...
            var renderSelector = function () {                
                ...
                $elem.parent().append($('<div class="modal fade GWUserOrgSelectorModal" id="' + $elem[0].id + 'Modal" tabindex="-1" role="dialog" aria-labelledby="' + $elem[0].id + 'Modal" aria-hidden="true">'
                    + ' <div class="modal-dialog">'
                    + '<div class="modal-content">'
                    + ' <div class="modal-body">'
                    + '  <button class="close btnColseGWUserOrgSelectorModal" id="btnClose' + $elem[0].id + 'Modal" title="关闭" data-dismiss="modal" type="button"  aria-hidden="true">&times;</button>'
                    + '     <iframe class="GWUserOrgSelectorFrame" id="' + $elem[0].id + 'Frame" ></iframe>'
                    + '  </div>'
                    + ' </div>'
                    + '</div>'
                    + '</div>'));
